...
export default function ClientHome({ user }: { user: GoogleUser | null }) {
  ...
  const submitQuestion = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    else if (/마켓|농산물|입고|장보기|나눔|무|다시다/.test(normalized)) reply = { text: "내일 마켓에는 무 10개(개당 3,000원)와 쇠고기 다시다 500g 10개(개당 11,000원)가 들어와유. 재고가 소진되기 전에 마켓에서 확인해 보세유.", actionLabel: "마켓 열기" };
  ...
  return (
    <main>
      ...
      {showMiniMarket && <div className="modal-backdrop" role="presentation" onMouseDown={() => setShowMiniMarket(false)}><section className="market-modal" role="dialog" aria-modal="true" aria-labelledby="market-title" onMouseDown={(event) => event.stopPropagation()}><button className="modal-close" type="button" aria-label="닫기" onClick={() => setShowMiniMarket(false)}>×</button><span className="mini-label">STAY MINI MARKET</span><h2 id="market-title">내일 입고될<br/>스테이 미니마켓</h2><p className="market-lead">스테이에 들어올 농산물과 나눔물품을 미리 확인하고, 입금 확인 뒤 다음 날 편하게 받아가세요.</p><div className="market-note">내일 판매 재고 총 20개 · 재고 소진 시 마감됩니다.</div><div className="market-groups"><section><div className="market-section-title"><span>📦</span><div><b>내일 입고 품목</b><small>가격·수량 확정</small></div></div><ul><li><span>🥬 무 1개</span><b>3,000원 · 10개</b></li><li><span>🧂 쇠고기 다시다 500g</span><b>11,000원 · 10개</b></li></ul></section><section><div className="market-section-title"><span>🧺</span><div><b>나눔물품</b><small>스테이 이용자 우선</small></div></div><ul><li><span>공용주방 기본양념</span><b>무료 나눔</b></li><li><span>재사용 장바구니</span><b>무료 나눔</b></li><li><span>식재료 보관용기</span><b>수량 한정</b></li></ul></section></div><div className="market-flow" aria-label="스테이 미니마켓 수령 순서"><span><b>1</b>오늘 신청</span><i>→</i><span><b>2</b>입고·금액 확정</span><i>→</i><span><b>3</b>다음 날 입금 확인 후 수령</span></div><p className="market-footnote">시범 운영 중이며, 품목·수량은 당일 입고 상황에 따라 달라질 수 있어요.</p></section></div>}
