...
export default function ClientHome({ user }: { user: GoogleUser | null }) {
  ...
  const saveJoin = async (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (!user) {
      window.location.href = "/api/auth/participant?return_to=/";
      return;
    }
    ...
    <main>
      <header className="topbar">
        ...
        {user
          ...
          : <a className="topbar-login" href="/api/auth/participant?return_to=/"><span>◎</span> 참여 시작하기</a>}
      </header>
        ...
        <section className="hero shell">
          <div className="hero-copy">
            ...
            <div className="hero-actions">
              <button className="primary" onClick={() => user ? move("join") : window.location.assign("/api/auth/participant?return_to=/")}>{user ? "Join 시작하기 →" : "로그인하고 시작하기 →"}</button>
      ...
      {tab === "place" && <LocalDiscovery displayName={displayName} signedIn={Boolean(user)} onRequireLogin={() => window.location.assign("/api/auth/participant?return_to=/")} />}
      {tab === "recipe" && <RecipeRoom signedIn={Boolean(user)} />}

      {tab === "join" && <section className="subpage shell">
        <div className="join-title-row"><div><span className="eyebrow">JOIN</span><h1>{joins.length}개의 홍성 Join</h1></div><button className="primary" onClick={() => user ? setCreatingJoin(true) : window.location.assign("/api/auth/participant?return_to=/")}>Join 만들기 <span>＋</span></button></div>
      ...
      {!user && <div className="modal-backdrop login-backdrop" role="presentation"><section className="login-modal" role="dialog" aria-modal="true" aria-labelledby="login-title"><img className="brand-icon" src="/brand/hongseong-station-ieum-icon.png" alt="" /><span className="mini-label">HONGSEONG MATE</span><h2 id="login-title">홍성에서<br/>함께할 준비 되셨나요?</h2><p>로그인 후 이용 유형을 설정하면, 홍성의 공간과 Join을 만날 수 있어요.</p><a className="primary" href="/api/auth/participant?return_to=/">간편 로그인하고 시작하기 →</a><small>기기 변경 또는 쿠키 삭제 전까지 참여 정보가 유지돼요.</small></section></div>}
