...
export default function ClientHome({ user }: { user: GoogleUser | null }) {
  ...
  const submitQuestion = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    else if (/(앱|어플|기능|메뉴|사용법|뭘할수|무엇을할수|이음이소개)/.test(normalized)) reply = { text: "홈에서는 구옥 공유 스테이와 오늘의 바로가기를 보고, 발견에서는 홍성 장소·축제·주차 정보를 살펴볼 수 있어유. 로컬마켓의 입고 예정·바로 구매 안내, 함께할 Join, 출처가 확인된 레시피, 로그인 후 프로필 설정도 이용할 수 있어유." };
    ...
    else if (/마켓|농산물|입고|장보기|나눔|무|다시다/.test(normalized)) reply = { text: "로컬마켓 입고 예정에는 무 10개(개당 3,000원)와 쇠고기 다시다 500g 10개(개당 11,000원)가 있어유. 지금 바로 구매 가능한 상품도 로컬마켓에서 함께 확인해 보세유.", actionLabel: "로컬마켓 열기" };
  ...
  return (
    <main>
      ...
      {tab === "home" && <>
        ...
        <section className="today-shortcuts shell" aria-labelledby="today-shortcuts-title">
          ...
          <div className="shortcut-grid">
            <button type="button" onClick={() => setShowMiniMarket(true)}><span className="shortcut-icon market">🧺</span><b>로컬마켓</b><small>입고 예정·바로 구매 보기</small><i>상품 보기</i></button>
      ...
      {showMiniMarket && <div className="modal-backdrop" role="presentation" onMouseDown={() => setShowMiniMarket(false)}><section className="market-modal" role="dialog" aria-modal="true" aria-labelledby="market-title" onMouseDown={(event) => event.stopPropagation()}><button className="modal-close" type="button" aria-label="닫기" onClick={() => setShowMiniMarket(false)}>×</button><span className="mini-label">HONGSEONG LOCAL MARKET</span><h2 id="market-title">홍성 로컬마켓</h2><p className="market-lead">입고될 상품을 미리 확인하고, 준비된 상품은 바로 구매할 수 있어요.</p><div className="market-list-tabs" role="tablist" aria-label="로컬마켓 상품 목록"><button type="button" role="tab" aria-selected={marketList === "arrival"} className={marketList === "arrival" ? "active" : ""} onClick={() => setMarketList("arrival")}><span>📦</span><b>입고 예정</b><small>2개 품목 · 총 20개</small></button><button type="button" role="tab" aria-selected={marketList === "available"} className={marketList === "available" ? "active" : ""} onClick={() => setMarketList("available")}><span>🛒</span><b>바로 구매</b><small>현재 0개 품목</small></button></div>{marketList === "arrival" ? <div className="market-list-panel" role="tabpanel"><div className="market-note">내일 입고 예정 · 재고 소진 시 마감됩니다.</div><ul className="market-product-list"><li><span className="market-product-icon">🥬</span><div><b>무 1개</b><small>내일 입고 예정</small></div><strong>3,000원<small>10개 예정</small></strong></li><li><span className="market-product-icon">🧂</span><div><b>쇠고기 다시다 500g</b><small>내일 입고 예정</small></div><strong>11,000원<small>10개 예정</small></strong></li></ul></div> : <div className="market-list-panel" role="tabpanel"><div className="market-empty"><span>🧺</span><b>현재 바로 구매 가능한 상품이 없어요.</b><p>상품이 입고되면 가격과 남은 수량, 구매 버튼이 여기에 표시됩니다.</p></div></div>}<div className="market-flow" aria-label="로컬마켓 이용 순서"><span><b>1</b>상품 확인</span><i>→</i><span><b>2</b>재고·금액 확인</span><i>→</i><span><b>3</b>입금 확인 후 수령</span></div><p className="market-footnote">시범 운영 중이며, 품목·수량은 실제 입고 상황에 따라 달라질 수 있어요.</p></section></div>}
